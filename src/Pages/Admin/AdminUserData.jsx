...
const AdminUserData = () => {
  ...
  const handleDone = async (user) => {
    try {
      dispatch(setLoading())
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/mark-done`, {
        _id: user._id,
        isDone: !user.isDone
      })
  ...
  const onDelete = async (user) => {
    try {
      dispatch(setLoading())
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/delete-user`, { _id: user._id })
      dispatch(hideLoading())
      if (response.data.success) {
        message.success(response.data.message)
        dispatch(setReloadData(true))
