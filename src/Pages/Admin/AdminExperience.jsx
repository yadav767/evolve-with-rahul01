...
const AdminExperience = () => {
  ...
  const submitHandler = async (values) => {
    ...
    try {
      ...
      if (selectedItemForEdit) {
        ...
        response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/update-experience`, formData, {
          headers: { "Content-Type": "multipart/form-data" }
        })
      } else {
        response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/add-experience`, formData, {
          headers: { "Content-Type": "multipart/form-data" }
        })
      }
  ...
  const onDelete = async (item) => {
    try {
      dispatch(setLoading())
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/delete-experience`, { _id: item._id })
      dispatch(hideLoading())
      if (response.data.success) {
        message.success(response.data.message)
        dispatch(hideLoading())
