...
const AdminBlog = () => {
  ...
  const submitHandler = async (values) => {
    ...
    try {
      ...
      if (selectedItemForEdit) {
        response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/update-blog`, { ...values, image, _id: selectedItemForEdit._id }, {
          headers: { "Content-Type": "multipart/form-data" }
        })
      } else {
        response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/add-blog`, { ...values, image }, {
          headers: { "Content-Type": "multipart/form-data" }
        })
      }
  ...
  const onDelete = async (item) => {
    try {
      dispatch(setLoading())
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/yoga/delete-blog`, { _id: item._id })
      dispatch(hideLoading())
      if (response.data.success) {
        message.success(response.data.message)
        dispatch(hideLoading())
